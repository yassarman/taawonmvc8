...
using TaawonMVC.Migrations.SeedData;
...
namespace TaawonMVC.Tests
{
    public abstract class TaawonMVCTestBase : AbpIntegratedTestBase<TaawonMVCTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaawonMVCDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaawonMVCDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaawonMVCDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaawonMVCDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaawonMVCDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaawonMVCDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaawonMVCDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaawonMVCDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaawonMVCDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaawonMVCDbContext, Task<T>> func)
        {
            T result;
